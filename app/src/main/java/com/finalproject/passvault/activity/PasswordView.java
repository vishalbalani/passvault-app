...
import com.finalproject.passvault.model.Password;
...
public class PasswordView extends AppCompatActivity implements View.OnClickListener {
        ...
        databaseReference.child(activity).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull @NotNull DataSnapshot snapshot) {
                try {
                    ...
                    password.setText(DecryptMessage(passwordView.getPassword()));
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btn_copy_username) {
            ...
            ClipData clip = ClipData.newPlainText("passvault username copy", username.getText().toString());
        ...
        if (v.getId() == R.id.btn_copy_password) {
            ClipboardManager copy = (ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
            ClipData clip = ClipData.newPlainText("passvault password copy", password.getText().toString());
            copy.setPrimaryClip(clip);
        ...
        if (v.getId() == R.id.btn_copy_note) {
            ClipboardManager copy = (ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
            ClipData clip = ClipData.newPlainText("passvault note copy", note.getText().toString());
            copy.setPrimaryClip(clip);
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    private String DecryptMessage(String enpassword) {
        byte[] decodedByte = Base64.getDecoder().decode(enpassword);
        String decryptedMsg = "";
        try {
